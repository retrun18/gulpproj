...
var vectorLayer1 = new ol.layer.Vector({
  source: new ol.source.Vector({                                                url: areasurl,
    format: new ol.format.GeoJSON()                                           }),                                                                         style: function(feature) {
	...
	getyiqing().then((yqjson)=>{
		...
		for(let key in json.features){                                                      let pro=json.features[key];                                                 item=meslist.find((mes)=>{                                                          return parseInt(mes.locationId)===parseInt(pro.properties.adcode);                                                                              })||{};
                        pro.properties.confirmedCount=item.confirmedCount||0;                                                                                                   pro.properties.curedCount=item.curedCount||0;                               pro.properties.deadCount=item.deadCount||0;                                                                                                    }	
		let vectorSource = new ol.source.Vector({
			features: (new ol.format.GeoJSON()).readFeatures(json,{
				featureProjection:'EPSG:3857'
 })
		});
		console.log((new ol.format.GeoJSON({dataProjection:"epsg:4326"})).readFeatures(json))

		let style = new ol.style.Style({                                      fill: new ol.style.Fill({                                                     color: 'rgba(255, 100, 100, 0.6)'                                         }),                                                                         stroke: new ol.style.Stroke({                                                 color: '#319FD3',                                                           width: 1                                                                  }),                                                                         text: new ol.style.Text({                                                     font: '12px Calibri,sans-serif',                                            fill: new ol.style.Fill({                                                     color: '#000'
    }),
    stroke: new ol.style.Stroke({                                                 color: '#fff',                                                              width: 3                                                                  })                                                                        })
});
		let vectorLayer = new ol.layer.Vector({
  source: vectorSource,
                    style: function(feature) {                                  style.getText().setText(feature.get('name'));
